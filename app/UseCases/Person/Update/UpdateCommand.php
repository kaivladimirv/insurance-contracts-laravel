    ...
    public static function rules(ValidationContext $context): array
    {
        return [
            'email' => [
                ...
                Rule::unique(Person::class, 'email')->where('company_id', $context->payload['company_id'])->ignore($context->payload['id'])
            ],
            'phone_number' => [
                'nullable',
                'max:15',
                'required_if:notifier_type,' . NotifierType::TELEGRAM->value,
                Rule::unique(Person::class, 'phone_number')->where('company_id', $context->payload['company_id'])->ignore($context->payload['id'])
            ],
            'notifier_type' => [
                'nullable',
